...
const initDevDocumentMcp = async (argv, options = {}) => {
  ...
  if (!args.skipSync) {
    const pending = await kneDocumentSync.listPendingSync({ homedir, apiUrl });
    if (pending.length === 0) {
      ...
      console.log(
        `同步完成：上传 ${syncSummary.synced}，跳过 ${syncSummary.skipped}，失败 ${syncSummary.failed}${
          args.dryRun ? `，待同步 ${syncSummary.pending}` : ''
        }`
      );
      if (syncSummary.failed > 0) {
        throw new Error('部分文件同步失败');
      }
    }
  }
