...
const generateManifest = async () => {
    ...
    await fs.ensureDir(path.resolve(output, 'readme'));
    ...
    const allManifest = lodash.transform(manifest, (result, value, key) => {
        result[key] = value.map((item) => {
            ...
            readmeTask.push({name: target.name, readme: target.readme});
    ...
    await Promise.all(Object.keys(allManifest).map(async (key) => {
        ...
        await fs.writeJson(path.resolve(output, `${key}.json`), value.map(({name, packageName, version}) => {
            return {
                name, packageName, version, readme: `readme/${key}_${name}.json`
            };
        }));
    }));

    await Promise.all(readmeTask.map(async ({name, readme}) => {
        await fs.writeJson(path.resolve(output, 'readme', name + '.json'), readme);
    }));
};
