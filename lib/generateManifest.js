...
const getManifestInfo = async () => {
  const packageJson = await fs.readJson(path.resolve(process.cwd(), process.env.INPUT_PATH || process.env.MANIFEST_FILE || 'package.json'));

  const manifest = {};

  await Promise.all(
    Object.keys(Object.assign({}, packageJson['manifest-config'])).map(async type => {
      manifest[type] = await Promise.all(
        packageJson['manifest-config'][type].map(async item => {
          const { name, options } = (() => {
...
const generateManifest = async () => {
    ...
    result[key] = value.map(item => {
      const target = Object.assign({}, item, {
        versions: lodash.transform(
          Object.values(item.versions).sort((a, b) => {
          ...
          (result, value) => {
            ...
            if (
              item.version === key ||
              (Array.isArray(item.options.importantVersion) && item.options.importantVersion.length > 0 && item.options.importantVersion.indexOf(key) > -1) ||
              Object.keys(result).length < (item.options.maxLength || 0)
            ) {
              result[key] = value;
            }
          },
  ...
  await Promise.all(
    readmeTask.map(async ({ name, readme }) => {
  ...
  await Promise.all(
    Object.keys(allManifest).map(async key => {
      const value = allManifest[key];
      await fs.writeJson(
        path.resolve(output, `list/${key}.json`),
        value.map(({ name, packageName, version, versions }) => {
          return {
            name,
            packageName,
            version,
            versions,
            readme: `readme/${key}/${name}.json`
          };
        })
      );
    })
