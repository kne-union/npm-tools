...
`);
  ...
  const useZip = outputs.length > 1;
  const zipPath = useZip
    ? options.out.toLowerCase().endsWith('.zip')
      ? options.out
      : `${options.out.replace(/\/+$/, '')}.zip`
    : null;
  ...
  if (!options.dryRun) {
    if (useZip) {
      ...
      item.outPath = path.join(options.out, item.filename);
      await fs.writeFile(item.outPath, item.content, 'utf8');
    }
  } else {
    outputs.forEach(item => {
      item.outPath = useZip ? zipPath : path.join(options.out, item.filename);
    });
  }

  console.log(`root: ${options.root}`);
  console.log(`out:  ${useZip ? zipPath : options.out}${options.dryRun ? ' (dry-run)' : ''}`);
  console.log(`files: ${outputs.length}${useZip ? ' (zipped)' : ''}`);
  outputs.forEach(item => {
  ...
  return {
    ...
    conflicts,
    root: options.root,
    out: useZip ? zipPath : options.out,
    zip: useZip ? zipPath : null
  };
};

module.exports = localeToI18n;
module.exports.parseArgs = parseArgs;
module.exports.flattenMessages = flattenMessages;
module.exports.escapeTarget = escapeTarget;
